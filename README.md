...
`ctrlrange="0. 1."`，属于已冻结的控制硬范围。官方离线仿真已进一步验证左右夹爪均为
`open=1.0`、`closed=0.1`，因此 `gripper_verified_in_official_environment=true`。该标定只确认
夹爪开闭方向和端点值，不等于默认启用执行器，也不证明对任意物体的稳定夹持效果；
`arm_planning.enabled` 仍为 `false`，其他未标定字段保持 `null`，默认抓放执行继续失败关闭。
...
- 机械臂KDL、速度、容差、轨迹时长和视觉验证窗口的官方仿真标定；夹爪开闭端点已验证，但默认
